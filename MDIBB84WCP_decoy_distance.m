...
function [protocolDescription,channelModel,leakageEC,parameters,solverOptions] = MDIBB84WCP_decoy_distance()
...
%     parameters.scan.thetaA = 0:pi/64:pi/4; %misalignment angle between Alice-Charlie
    parameters.scan.eta = flip(10.^(-2:0.25:0)); %channel transmittance between Alice-Charlie (half of total distance)
...
%     parameters.fixed.eta = 1; %channel transmittance between Alice-Charlie (half of total distance)
    parameters.fixed.thetaA = 0.15; %misalignment angle between Alice-Charlie
    parameters.fixed.thetaB = -0.15; %misalignment angle between Bob-Charlie

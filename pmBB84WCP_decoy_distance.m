...
function [protocolDescription,channelModel,leakageEC,parameters,solverOptions] = pmBB84WCP_decoy_distance()
...
function parameters=setParameters()
    ...
    parameters.scan.eta = flip(10.^(-2:0.25:0)); %channel transmittance

    %%%%%%%%%%%%%%%% 2.fixed parameters %%%%%%%%%%%%%%%%
    %optional; the constant values can be either numerical or array/matrices
    
    parameters.fixed.ed = sin(0.3)^2; %misalignment, defined as single-photon error, i.e. sin^2(theta)
    ...
    parameters.optimize.mu1 = [0.2,0.3,0.4]; %can optionally optimize laser intensity (need to comment it out mu1 in 'fixed' category)
